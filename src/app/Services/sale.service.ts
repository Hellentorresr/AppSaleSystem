...
export class SaleService {
  ...
  History(searchBy: string, saleNumber: string, startDate: string, endDate: string): Observable<ResponseApi> { //RESPONSEAPI is the response structure I declared
    ...
    return this.http.get<ResponseApi>(`${this.urlApi}SaleHistory?searchBy=${searchBy}&saleNumber=${saleNumber}&startDate=${startDate}&endDate=${endDate}`); //the final endpoint
  }

  Report(startDate: string, endDate: string): Observable<ResponseApi> { //RESPONSEAPI is the response structure I declared

    return this.http.get<ResponseApi>(`${this.urlApi}Report?startDate=${startDate}&endDate=${endDate}`); //the final endpoint
  }
}
